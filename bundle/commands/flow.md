---
description: Plan, implement, and complete trackable work via the orchestrator (subcommand router)
...
2. **Dispatch the `planner` agent.** Its prompt must include:
   ...
   - The discovery it must perform: features → layers + affected files; remediations → read
