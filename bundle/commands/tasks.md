...
**Examples:**
- `/tasks Add presence tracking to chat sessions`
- `/tasks Remediate all confirmed findings in .claude/sweep/auth/`
- `/tasks Implement the import feature from docs/IMPORT_SPEC.md`
...
2. **Read context.** Before planning, read:
   ...
   - `.claude/rules/*.md` — layering and quality conventions, if the project ships them.
...
- Identify the layers involved (schema/models, services, data access, entry points, serialization, tests, wiring/registration, migrations).
...
- Identify all sites that need changes (grep, read, trace references).
...
4. **Grouping rules:**
   ...
   - **Infrastructure first.** Schema/models, migrations, and shared utilities are Phase 0.
   - **Core logic next.** Services/use-cases, data access, and authorization are Phase 1.
   - **Integration last.** Entry points, serialization, wiring, and registration are Phase 2 (or later).
...
## Validation
```bash
<specific commands to verify this subtask>
```
