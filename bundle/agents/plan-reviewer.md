...
> Stack-agnostic. Read `.claude/CONTEXT_MAP.md` and `.claude/rules/*.md` first — they
> define the project's layering, conventions, and validation commands. Judge the plan
> against those, not against generic ideals.
...
2. **Ownership completeness.** Every file the implementation steps mention must appear in
   some subtask's `Files Owned`. Flag files that are edited-but-unowned, and owned-but-unused.
...
4. **Phase ordering.** Infrastructure (schema/models/migrations/shared utils) before core
   logic before integration before cross-review. Flag anything that depends on output it
   can't have yet, and any migration sequenced after the code that needs it.
...
7. **Test & validation coverage.** Every subtask that changes behavior needs tests (happy
   path, failure path, authorization/boundary) and a runnable validation command. Flag gaps.
   Confirm the `## Validation gate` command is the project's real one (per CONTEXT_MAP/rules).
