...
- **Local modifications:** data/ and scripts/ symlinks resolved to inline content; otherwise verbatim.
...
## ui-ux-pro-max specifics
Symlinks in upstream (`data` → `../../../src/ui-ux-pro-max/data`, `scripts` →
`../../../src/ui-ux-pro-max/scripts`) were resolved during import: the actual content
from upstream `src/ui-ux-pro-max/` was placed directly under `data/` and `scripts/`
in this kit. `python3 scripts/search.py` works as documented; `Path(__file__).parent.parent / "data"`
resolves correctly under our flattened layout.